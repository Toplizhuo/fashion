...
LEARNING_RATE = 0.001
DROPOUT_RATE = 0.1
ckpt_path = 'results/CNNModel/2CNN+3FC+BN+dropout/2CNN+3FC+BN+dropout.ckpt'
...
def conv_layer(input_value, size_in, size_out, kernel_size, strides, name):
    with tf.name_scope(name):
        ...
        # normalization
        conv = normalization(conv, size_out, [0, 1, 2], training=on_train)
...
def fc_layer(input_value, size_in, size_out, name, use_relu=True, use_norm=True):
    with tf.name_scope(name):
        ...
        if use_norm:
            # normalization
            fc_output = normalization(fc_output, size_out, [0], training=on_train)
        if use_relu:
...
def normalization(input_value, output_size, axes, training=True):
    with tf.name_scope('normalization'):
        fc_mean, fc_var = tf.nn.moments(input_value, axes=axes)
        ...
        mean, var = tf.cond(training, mean_var_with_update, lambda: (ema.average(fc_mean), ema.average(fc_var)))
        output = tf.nn.batch_normalization(input_value, mean, var, shift, scale, epsilon)
        return output
...
on_train = tf.placeholder(tf.bool, None)  # to control dropout and normalization when training and testing
...
h_fc1 = fc_layer(h_pool2_flat, 7*7*64, 1024, 'hidden_layer1', use_norm=True)
# dropout here
h_fc1 = tf.layers.dropout(h_fc1, rate=DROPOUT_RATE, training=on_train, name='dropout1')
# fc layer 2: hidden layer2
h_fc2 = fc_layer(h_fc1, 1024, 1024, 'hidden_layer2', use_norm=True)
# dropout here
h_fc2 = tf.layers.dropout(h_fc2, rate=DROPOUT_RATE, training=on_train, name='dropout2')
# fc layer 3: output layer
prediction = fc_layer(h_fc2, 1024, 10, 'output_layer', use_relu=False, use_norm=False)
...
writer = tf.summary.FileWriter("results/CNNModel/2CNN+3FC+BN+dropout/", sess.graph)
...
if not os.path.exists('results/CNNModel/2CNN+3FC+BN+dropout/checkpoint'):  # if no model to restore
    ...
    for step in range(EPOCHS):
        ...
        sess.run(train_step,
                 feed_dict={xs: batch_xs, ys: batch_ys, on_train: True},
                 options=None,
                 run_metadata=None)
        if (step+1) % 200 == 0:
            current_accuracy = sess.run(accuracy, feed_dict={xs: X_VAL, ys: Y_VAL, on_train: False})
            train_result = sess.run(merged, feed_dict={xs: X_VAL, ys: Y_VAL, on_train: False})
    ...
    many_runs_timeline.save('results/CNNModel/2CNN+3FC+BN+dropout/timeline_merged_of_%d_iterations.json' % 5)
    address = saver.save(sess, ckpt_path)
    print("Finish training, we'v stored the model's parameters to", address)
    final_accuracy = sess.run(accuracy, feed_dict={xs: X_TEST, ys: Y_TEST, on_train: False})
